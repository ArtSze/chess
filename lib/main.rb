...
g6 = game.retrieve_square("G6")
g6.occupied_by = piece_two
piece_two.current_square = g6.co_ord
...
d7 = game.retrieve_square('D7')
d7.occupied_by = piece_four
piece_four.current_square = d7.co_ord
...
def check_path(piece, game)
  ...
  taken.each do |square|
    ...
    # duplicate for opp color with to_omit taking > instead of >=
    if game.retrieve_square(square).occupied_by.color == piece.color
      if up_left_diagonal.include?(square)
        to_omit = piece.moves.select do |move|
          up_left_diagonal.include?(move) && move.last.to_i >= taken_row_num
        end
        piece.moves.delete_if { |move| to_omit.include?(move) }
      elsif up_right_diagonal.include?(square)
        to_omit = piece.moves.select do |move|
          up_right_diagonal.include?(move) && move.last.to_i >= taken_row_num
        end
        piece.moves.delete_if { |move| to_omit.include?(move) }
      elsif down_left_diagonal.include?(square)
        to_omit = piece.moves.select do |move|
          down_left_diagonal.include?(move) && move.last.to_i <= taken_row_num
        end
        piece.moves.delete_if { |move| to_omit.include?(move) }
      elsif down_right_diagonal.include?(square)
        to_omit = piece.moves.select do |move|
          down_right_diagonal.include?(move) && move.last.to_i <= taken_row_num
        end
        piece.moves.delete_if { |move| to_omit.include?(move) }
      elsif (taken_column_ascii < piece_column_ascii) && (taken_row_num == piece_row_num) #left
        to_omit = piece.moves.select do |move|
          (move.first.ord <= taken_column_ascii) && (move.last.to_i == taken_row_num)
        end
        piece.moves.delete_if { |move| to_omit.include?(move) }
...
def king_viable_moves #will check if any moves within king's move_set put him into check
end
